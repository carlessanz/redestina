// El formulario de subida de la factura del donante. Se usa en DOS sitios y es el mismo
// código:
//
//   · `/factura/:token`, la página pública — la abre el donante desde el correo con su
//     resumen anual, casi siempre en el móvil y sin cuenta.
//   · Dentro de un diálogo, desde el panel del equipo — la subida **asistida**, que el
//     dinamizador conduce con la persona al teléfono (`acunar_enllac_assistit`, §9).
//
// Hermano de `FormulariConfirmacio` y de `FirmaConveni`, y partido por lo mismo: no puede
// haber dos formularios. Lo que se sube y la evidencia que deja son los mismos por los dos
// caminos; lo único que cambia es el marco, y lo dice `ample`.
//
...
//    token.
//
// LO QUE DECIDE LO DE ABAJO es que quien lo abre no tiene cuenta: fuera de `RequireSessio`,
// sin `AppContextProvider`, una columna, controles de 44 px, `text-base` en todo lo que se
// enfoca, y un mensaje propio para cada final del enlace (no existe, ya usado, caducado)
// porque lo que hay que hacer después es distinto en cada caso.
//
...
export interface PropsFormulariFactura {
  token: string | undefined
  /** `true` = hay sitio (diálogo). No es «pantalla grande»; ver `FormulariConfirmacio`. */
  ample?: boolean
  /** Lo llama al subir con éxito. El diálogo lo usa para refrescar el panel de detrás. */
  onPujada?: () => void
}

export default function FormulariFactura(
  { token, ample = false, onPujada }: PropsFormulariFactura,
) {
  const { t } = useT()
  ...
  if (!dades || errorKey) {
    return (
      <div className="space-y-3">
        <h2 className="text-lg font-semibold">{t('fact.problem')}</h2>
    ...
    return (
      <div className="space-y-3">
        <h2 className="text-lg font-semibold">
          {t(cuadra ? 'fact.done_title' : 'fact.done_title_check')}
        </h2>
        <p className="text-sm">
  ...
  return (
    <div className="space-y-5">
      {/* En el diálogo el título lo pone su propia cabecera, así que aquí sobraría; la
          referencia del resumen no, que es lo que dice DE QUÉ cierre se está hablando. */}
      <div>
        {!ample && <h2 className="text-lg font-semibold">{t('fact.title')}</h2>}
      ...
      {/* Lo dice ANTES de que se suba nada: la subida va a quedar etiquetada como asistida
          en la evidencia, con el nombre de quien la conduce. Lo decide el servidor por
          `enlaces_token.canal`, no esta pantalla. */}
      {dades.assistida && (
        <p className="rounded-md bg-secondary p-3 text-sm text-secondary-foreground">
          {t('fact.assisted_note')}
        </p>
      )}
